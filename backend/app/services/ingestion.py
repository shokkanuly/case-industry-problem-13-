...
async def process_telemetry(packet: TelemetryPacket) -> TelemetryResponse:
    """
    Decoupled Ingestion:
    Publishes the raw telemetry packet to the message broker and returns immediately.
    The database writes, state calculations, and alerting run asynchronously.
    """
    server_ts = int(time.time())
    broker = get_message_broker()
    
    # Publish raw packet to broker topic
    await broker.publish("industrial/telemetry/raw", packet.dict())
...
async def process_telemetry_core(packet: TelemetryPacket) -> None:
    """
    Asynchronous Core processing pipeline executed by the broker subscriber.
    1. Writes raw high-frequency telemetry to TelemetryStore (DuckDB / InfluxDB).
    2. Updates devices registry in SQLite.
    3. Looks up device_id in Case Registry.
    4. Routes processing by output_type (status, recommendation, batch_report).
    5. Detects status transitions and logs to 'alerts' table.
    6. Upsert Digital Twin ('assets') table in SQLite.
    7. Publishes updates & alerts to the message broker.
    """
    server_ts = int(time.time())
    device_ts = packet.timestamp if packet.timestamp else server_ts

    # ── Step 1: Raw Telemetry Log (Time-Series Store) & SQLite Device Tracker ──
    try:
        ...
        logger.error(f"Failed to write to time-series database: {e}")
    ...
    if case_entry:
        ...
        try:
            ...
            logger.error(f"Failed to fetch previous status: {e}")
        ...
        if status != prev_status:
            ...
            # Publish Alert to Message Broker
            ...
            if asset_id == "haul_road_zone_b" and status in ["Warning", "Critical"]:
                ...
                async def run_gemini_vision(aid=alert_id, lsum=label_summary):
                    ...
                    # Broadcast description updates via Message Broker
                    await broker.publish("industrial/alerts", {
                    ...
                    try:
                        ...
                        logger.error(f"Failed to update alert message with Gemini description: {e}")
        ...
        try:
            ...
            logger.error(f"Failed to update asset digital twin: {e}")

        # Publish Digital Twin update to Message Broker
